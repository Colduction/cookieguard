...
type Config struct {
	...
	// Return the original cookie data and suppress errors.
	//
	// Optional. Default: false
	ReturnOriginalOnError bool
}
...
func configDefault(config ...Config) Config {
	...
	if len(config) > 0 {
		...
		if !cfg.EncryptKey && !cfg.EncryptValue {
			cfg.EncryptValue = ConfigDefault.EncryptValue
		}
	}
